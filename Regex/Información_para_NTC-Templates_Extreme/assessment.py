...
import pandas as pd
from parseToTextfsm import getDevicesData
from pprint import pprint
import json
            ...
            if "device_dataframe" == df_type:
                ...
                for command, df_cmd in dataframes[df_type].items():
                    ...
                    try:
                        ...
                        print(f"ERROR DATAFRAME COMMAND-> {command}")
                writer.save()
...
customer = Assessment("Extreme")
devices_data = getDevicesData()
devices_data = customer.create_folder_structure("extreme_exos", devices_data)
dataframes = create_dataframes(devices_data)

write("dataframes", customer.project_folder, devices_data)

customer.write_to_excel("extreme_exos", dataframes)
